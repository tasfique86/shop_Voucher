...
// Stationery preset items for quick insertion
const STATIONERY_PRESETS = [
  { nameBangali: 'খাতা (Notebook)', nameEnglish: 'Notebook', rate: 60 },
  { nameBangali: 'বলপেন (Ballpoint Pen)', nameEnglish: 'Ballpoint Pen', rate: 10 },
  { nameBangali: 'এ ফোর পেপার (A4 Paper Box)', nameEnglish: 'A4 Paper Box', rate: 450 },
  { nameBangali: 'ফটোকপি পেপার (Photocopy Paper)', nameEnglish: 'Photocopy Paper (Ream)', rate: 380 },
  { nameBangali: 'সায়েন্টিফিক ক্যালকুলেটর (Calculator)', nameEnglish: 'Scientific Calculator', rate: 1200 },
  { nameBangali: 'জ্যামিতি বক্স (Geometry Box)', nameEnglish: 'Geometry Box', rate: 150 },
  { nameBangali: 'ফাইল ফোল্ডার (File Folder)', nameEnglish: 'File Folder', rate: 45 },
  { nameBangali: 'মার্কার পেন (Marker Pen)', nameEnglish: 'Marker Pen', rate: 35 },
  { nameBangali: 'পেন্সিল বক্স (Pencil Box)', nameEnglish: 'Pencil Box', rate: 80 },
...
export default function VoucherPage() {
  ...
  const [customRemarks, setCustomRemarks] = useState('পণ্য ক্রয়ের ৭ দিনের মধ্যে পরিবর্তনযোগ্য।');

  // Form Validation & Errors
  const [errors, setErrors] = useState<{
    customerName?: string;
    products?: string;
    productRows?: { [key: string]: { description?: string; quantity?: string; rate?: string } };
  }>({});
  ...
  const handleVoucherNumberChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleRowChange = (id: string, field: keyof ProductRow, value: any) => {
    updateVoucher((prev) => {
      const updatedProducts = prev.products.map((p) => {
        if (p.id === id) {
          const newVal = field === 'description' ? value : parseFloat(value) || 0;
  ...
  const deleteProductRow = (id: string) => {
    if (voucher.products.length <= 1) {
      setErrors((prev) => ({ ...prev, products: "ভাউচারে অন্তত একটি পণ্য থাকতে হবে।" }));
      return;
    }
    updateVoucher((prev) => {
  ...
  const insertPresetItem = (preset: typeof STATIONERY_PRESETS[0]) => {
    // Find an empty row to replace, or append a new one
    const emptyRowIdx = voucher.products.findIndex(p => p.description.trim() === '' && p.quantity === 0 && p.rate === 0);
    const desc = showEnglishLabels ? `${preset.nameBangali} / ${preset.nameEnglish}` : preset.nameBangali;
  ...
  const validateForm = (): boolean => {
    ...
    if (!voucher.customerName.trim()) {
      currentErrors.customerName = "ক্রেতার নাম আবশ্যক (Customer Name is required)";
      isValid = false;
    }

    // Validate product rows count
    const activeProducts = voucher.products.filter(p => p.description.trim() !== '');
    if (activeProducts.length === 0) {
      currentErrors.products = "ভাউচারে অন্তত একটি মালের বিবরণ থাকতে হবে (At least one valid product is required)";
      isValid = false;
    }

    // Validate quantities and rates
    const productRowErrors: { [key: string]: { description?: string; quantity?: string; rate?: string } } = {};
    voucher.products.forEach(p => {
      if (p.description.trim() !== '') {
        const rowErr: { description?: string; quantity?: string; rate?: string } = {};
        if (p.quantity <= 0) {
  ...
  const handleFinalize = (e: React.FormEvent) => {
    e.preventDefault();
    if (!validateForm()) {
      alert("দয়া করে ভুলের স্থানগুলো সংশোধন করুন। (Please correct form validation errors before proceeding)");
      return;
    }

    finalizeVoucherAndIncrement();
    alert("ভাউচারটি সফলভাবে সংরক্ষণ করা হয়েছে এবং নতুন ভাউচার নম্বর বরাদ্দ করা হয়েছে! (Voucher finalized successfully!)");
  };

  // Keyboard friendly quick print
  const handlePrint = () => {
    if (!validateForm()) {
      const confirmPrint = window.confirm("ভাউচারে কিছু তথ্য অসম্পূর্ণ রয়েছে। তবুও কি প্রিন্ট করতে চান? (Some fields are missing. Print anyway?)");
    ...
    <div className="min-h-screen bg-slate-100 text-slate-800 flex flex-col justify-between font-sans selection:bg-indigo-500 selection:text-white">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold tracking-tight flex items-center gap-1.5 text-slate-900">
                ...
                <span className="text-xs bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded-full border border-indigo-100/50 font-mono font-medium">v1.2</span>
              </h1>
              <p className="text-xs text-slate-500 font-medium">Sonali Paper & Stationery - Voucher Generator System</p>
            </div>
          </div>
      ...
      <main className="flex-grow max-w-7xl w-full mx-auto px-4 py-6 md:py-8 grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8">
        ...
        <div className="lg:col-span-5 space-y-6 no-print">
          ...
          {showPresetInfo && (
            <div className="bg-white border border-slate-200 p-4 rounded-xl shadow-md relative animate-in fade-in slide-in-from-top-4 duration-200">
              ...
              <h3 className="text-sm font-bold text-indigo-600 flex items-center gap-1.5 mb-2 uppercase tracking-wide">
                <Sparkles className="w-4 h-4" />
                দ্রুত পণ্য সংযোজন (Quick Stationery Presets)
              </h3>
              <p className="text-xs text-slate-500 mb-3">
                পণ্যটিতে ক্লিক করলেই তা সাথে সাথে ভাউচারে যোগ হয়ে যাবে এবং মূল্য স্বয়ংক্রিয়ভাবে হিসাব হবে।
              </p>
              <div className="flex flex-wrap gap-2 max-h-[180px] overflow-y-auto pr-1">
                {STATIONERY_PRESETS.map((p, idx) => (
                  <button
                    key={idx}
                  ...
                  >
                    ...
                    <span className="opacity-70 font-mono font-medium">({p.rate}৳)</span>
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="bg-white rounded-xl p-5 md:p-6 border border-slate-200 shadow-sm space-y-6">
            ...
            <form onSubmit={handleFinalize} className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-400 mb-1 uppercase tracking-wider">
                    ক্রেতার নাম (Customer Name) <span className="text-red-500 font-bold">*</span>
                  </label>
                  <input
                    type="text"
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-3 max-h-[360px] overflow-y-auto pr-1">
                  {voucher.products.map((row, index) => (
                    ...
                    >
                      {/* Row Header Indicator & Trash */}
                      <div className="flex justify-between items-center">
                        <span className="text-[10px] font-bold font-mono text-slate-400 bg-slate-200/50 px-2 py-0.5 rounded border border-slate-200">
                          ROW #{index + 1}
                        </span>
                        ...
                        <button
                          ...
                          className="text-slate-400 hover:text-red-500 disabled:opacity-30 disabled:hover:text-slate-400 p-1 rounded hover:bg-slate-200/50 cursor-pointer"
                      ...
                      <div className="grid grid-cols-12 gap-2">
                        ...
                        <div className="col-span-4 sm:col-span-2">
                          <input
                            ...
                            placeholder="পরিমাণ"
                            className={`w-full bg-white border ${errors.productRows?.[row.id]?.quantity ? 'border-red-500' : 'border-slate-200'} focus:border-indigo-500 focus:ring-1 rounded-md px-2 py-1.5 text-xs text-center font-mono text-slate-800 shadow-sm`}
                        ...
                        <div className="col-span-4 sm:col-span-2">
                          <input
                            ...
                            placeholder="দর"
                            className={`w-full bg-white border ${errors.productRows?.[row.id]?.rate ? 'border-red-500' : 'border-slate-200'} focus:border-indigo-500 focus:ring-1 rounded-md px-2 py-1.5 text-xs text-center font-mono text-slate-800 shadow-sm`}
                            title="Rate (positive number)"
                          />
                        </div>

                        {/* Total Taka Output */}
                        <div className="col-span-4 sm:col-span-2 flex items-center justify-end px-2 font-mono font-bold text-xs text-indigo-600 bg-indigo-50/50 rounded border border-indigo-100/40">
                      ...
                      {errors.productRows?.[row.id] && (
                        <div className="text-[10px] text-red-500 flex justify-between px-1">
                          {errors.productRows[row.id].quantity && <span>পরিমাণ: {errors.productRows[row.id].quantity}</span>}
                          {errors.productRows[row.id].rate && <span>দর: {errors.productRows[row.id].rate}</span>}
                        </div>
                      )}
                    </div>
                  ))}
              ...
              <div className="border-t border-slate-100 pt-4 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3 bg-slate-50 p-2.5 rounded-lg border border-slate-200">
                  ...
                  <label className="flex items-center gap-2 cursor-pointer select-none">
                    ...
                    />
                    <span className="text-xs font-semibold text-slate-700">ইংরেজী লেবেল সহ</span>
                  </label>
                  ...
                  <label className="flex items-center gap-2 cursor-pointer select-none">
                    <input
                      ...
                      className="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 bg-white"
                    />
                    <span className="text-xs font-semibold text-slate-700">বাংলা সংখ্যা ফরম্যাট</span>
                  </label>
                </div>
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-400 mb-1 uppercase tracking-wider">
                    শর্তাবলী / বিশেষ মন্তব্য (Footer Remarks)
                  </label>
                  <input
                    type="text"
                    value={customRemarks}
        ...
        <div className="lg:col-span-7 flex flex-col items-center">
            ...
            >
              ...
              <div className="relative z-10 flex-grow flex flex-col justify-between">
                <div>
                  ...
                  <ProductTable 
                    products={voucher.products.filter(p => p.description.trim() !== '' || p.quantity > 0 || p.rate > 0)}
          ...
          <div className="hidden print:block print:absolute print:inset-0 print:bg-white print:text-black w-full" style={{ position: 'absolute', top: 0, left: 0 }}>
            <div 
          ...
          <div className="bg-white border border-slate-200 rounded-xl max-w-2xl w-full shadow-lg overflow-hidden animate-in zoom-in-95 duration-150 text-slate-800">
            ...
            <div className="bg-slate-50 px-5 py-4 border-b border-slate-200 flex justify-between items-center">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">লোকাল স্টোরেজে সংরক্ষিত বিগত ক্যাশ মেমোর তালিকা</p>
              </div>
              <button 
              ...
              <div className="max-h-[300px] overflow-y-auto space-y-2 pr-1">
                {filteredHistory.length === 0 ? (
                  <div className="text-center py-8 text-slate-500 text-xs font-semibold">
                    কোন সংরক্ষিত ভাউচার পাওয়া যায়নি। (No records match the search)
                  </div>
                ) : (
                    ...
                    >
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-bold text-slate-800 text-xs">{item.customerName}</span>
                        </div>
                        <div className="text-[10px] text-slate-500 mt-1 flex gap-3">
                          <span>তারিখ: {item.date}</span>
                          <span>পরিমাণ: {item.products.filter(p => p.description.trim() !== '').length}টি পণ্য</span>
                          <span className="font-mono font-bold text-indigo-600">মোট: {item.totalAmount}৳</span>
                        </div>
                      </div>
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          onClick={() => {
                            if (confirm(`আপনি কি সত্যিই ভাউচার ${item.voucherNumber} মুছে ফেলতে চান?`)) {
            ...
            <div className="bg-slate-50 px-5 py-3 border-t border-slate-200 text-right text-[11px] text-slate-500">
              পরবর্তী বরাদ্দকৃত নম্বর: <strong className="font-mono text-indigo-600 font-bold">{getNextVoucherNumberStr()}</strong>
            </div>
          </div>
        </div>
      )}
      ...
      <footer className="bg-white py-5 text-center border-t border-slate-200 text-xs text-slate-500 no-print mt-12">
        <div className="max-w-7xl mx-auto px-4">
          <p>© {new Date().getFullYear()} সোনালী পেপার এন্ড স্টেশনারী। সর্বস্বত্ব সংরক্ষিত।</p>
          <p className="text-[10px] text-slate-400 mt-0.5">Designed with A4 printing pixel accuracy and automatic offline cache integration.</p>
        </div>
      </footer>
