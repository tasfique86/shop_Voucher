...
export default function ProductTable({
  ...
  return (
    <div className="w-full border-2 border-black rounded-none overflow-hidden mb-4" id="product-table-print">
        ...
        <thead>
          <tr className="bg-black text-white text-xs md:text-sm font-bold border-b-2 border-black text-center">
            ...
            <th className="py-2 px-1 border-r border-white w-[10%] shrink-0">
              <div>ক্র: নং</div>
              {showEnglishLabels && <div className="text-[10px] font-normal italic">Sl. No.</div>}
            </th>

            {/* Description of Goods */}
            <th className="py-2 px-3 border-r border-white w-[50%] text-left">
              <div>মালের বিবরণ</div>
              {showEnglishLabels && <div className="text-[10px] font-normal italic">Description of Goods</div>}
            </th>

            {/* Quantity */}
            <th className="py-2 px-1 border-r border-white w-[12%] text-center">
              <div>পরিমাণ</div>
              {showEnglishLabels && <div className="text-[10px] font-normal italic">Qty</div>}
            </th>

            {/* Rate */}
            <th className="py-2 px-1 border-r border-white w-[13%] text-center">
              <div>দর</div>
              {showEnglishLabels && <div className="text-[10px] font-normal italic">Rate</div>}
            </th>

            {/* Amount (Taka) */}
            <th className="py-2 px-3 w-[15%] text-right">
              <div>টাকা</div>
              {showEnglishLabels && <div className="text-[10px] font-normal italic">Amount</div>}
            </th>
          </tr>
        </thead>
        <tbody>
          ...
          <tr className="border-t-2 border-black bg-gray-50/80 h-9 font-bold">
            <td colSpan={3} className="border-r border-black py-1 px-3">
              {/* Left bottom can be empty */}
            </td>
            ...
            <td className="bg-black text-white text-center border-r border-black py-1 text-xs md:text-sm font-extrabold flex items-center justify-center h-full min-h-[36px] uppercase">
              <span>মোট</span>
              {showEnglishLabels && <span className="text-[10px] font-normal italic ml-1">(Total)</span>}
            </td>

            {/* Total Amount Value */}
            <td className="text-right px-3 font-mono font-extrabold text-sm md:text-base text-black bg-gray-100/90">
              {useBengaliDigits ? toBengaliNumber(totalAmount.toFixed(2)) : totalAmount.toFixed(2)}
            </td>
          </tr>
        </tbody>
      </table>
