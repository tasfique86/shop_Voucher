...
export default function VoucherHeader({ voucherNumber, date, showEnglishLabels }: VoucherHeaderProps) {
  ...
  return (
    <div className="relative border-b-2 border-black pb-4 mb-4" id="voucher-header">
      ...
      <div className="absolute top-0 right-0 transform translate-x-2 -translate-y-2 bg-black text-white text-[11px] font-bold tracking-wider px-3 py-1 shadow-sm border border-black uppercase">
        ক্যাশ মেমো <span className="text-[9px] font-mono block text-center font-normal">CASH MEMO</span>
      </div>

      {/* Main Header Row */}
      <div className="text-center pt-3">
        <h1 className="text-3xl md:text-4xl font-extrabold text-black tracking-tight mb-1 font-sans">
          সোনালী পেপার এন্ড স্টেশনারী
        </h1>
        <p className="text-xs md:text-sm font-bold text-black max-w-3xl mx-auto leading-relaxed px-4">
          এখানে যাবতীয় অফিস, স্কুল কলেজের খাতা-কলম, কাগজ ফটোকপি পেপার ও মেশিনের কালি, ক্যালকুলেটর ও স্টেশনারী মালামাল পাইকারী ও খুচরা বিক্রয় করা হয়।
        </p>
        {showEnglishLabels && (
          <p className="text-[10px] text-gray-500 italic mt-0.5">
            Sonali Paper & Stationery — Premium wholesale & retail supplier of stationery, paper books & calculators.
          </p>
        )}
      </div>
      ...
      <div className="mt-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex items-end gap-1.5">
          ...
          {showEnglishLabels && <span className="text-[10px] text-gray-500 italic">(No.)</span>}
        </div>

        {/* Date Boxes matching the image */}
        <div className="flex items-center gap-2">
          <span className="font-extrabold text-black text-base">তারিখ :</span>
          {showEnglishLabels && <span className="text-[10px] text-gray-500 italic mr-1">(Date)</span>}
